...
try:
    ...
    from EstimatorVecs.models.word2vec_inner import train_batch_sg, train_batch_cbow
    from EstimatorVecs.models.word2vec_inner import score_sentence_sg, score_sentence_cbow
    from EstimatorVecs.models.word2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH

except ImportError:
    # failed... fall back to plain numpy (20-80x slower training than the above)
    print('using python version w2v')
    FAST_VERSION = -1
    ...
    def train_batch_sg(model, sentences, alpha, work=None, compute_loss=False):
        ...
        This is the non-optimized, Python version. If you have cython installed, EstimatorVecs
        will use the optimized version from word2vec_inner instead.
        """
        print(model.wv.syn0[0])
        print(model.syn1neg[0])
        print('parallell!!!')
        np.random.seed(1)
        result = 0
        train_pair_calls = 0
        for sentence in sentences:
            print('a'+str(model.wv.vocab['the'].index))
            #print(len(sentence))
            qq_list = []
            for qq in sentence[0:20]:
                if qq in model.wv.vocab:
                    qq_list = qq_list + [model.wv.vocab[qq].index]
                else:
                    qq_list = qq_list + ['oov']
            #print(qq_list)
            '''
            ...
            word_vocabs = [model.wv.vocab[w] for w in sentence if w in model.wv.vocab and
                           model.wv.vocab[w].sample_int > np.random.random() * 2 ** 32]
            #print('kept words in batch '+str(len(word_vocabs)))
            #print(np.random.random())
            #print('cat'+9)
            qq_list = []
            for qq in word_vocabs[0:20]:
                qq_list = qq_list + [qq.index]
            print(qq_list)
            print(len(word_vocabs))
            
            #print(np.random.randint(100))
            #print(np.random.random())
            for pos, word in enumerate(word_vocabs):
                #print('---')
                #print(np.random.random())  #same!
                #print('wi: '+str(word.index))
                ...
                #print(str(pos)+' '+str(word.index)+' '+str(reduced_window)+' '+str(start)+' '+str((pos + model.window + 1 - reduced_window)))
                ...
                for pos2, word2 in enumerate(word_vocabs[start:(pos + model.window + 1 - reduced_window)], start):
                    ...
                    if pos2 != pos:
                        ...
                        #if word == word2:
                            #print('how3? '+str(pos)+' '+str(word.index)+' '+str(pos2)+' '+str(word2.index))
                        #print(np.random.randint(100))
                        train_sg_pair(
                            model, model.wv.index2word[word.index], word2.index, alpha, compute_loss=compute_loss
                        )
                        #print(np.random.randint(100))
                        train_pair_calls = train_pair_calls + 1
                        #print('wj '+ str(word2.index)+' '+str(pos2))

            result += len(word_vocabs)
            
        print('train_pair_calls'+str(train_pair_calls))
        return result
...
def train_sg_pair(model, word, context_index, alpha, learn_vectors=True, learn_hidden=True,
    ...
    if model.negative:
        ...
        #print(str(predict_word.index)+' '+str(context_index)+' '+str(word_indices[1])+' '+str(word_indices[2])+' '+str(word_indices[3])+' '+str(word_indices[4])+' '+str(word_indices[5]))
