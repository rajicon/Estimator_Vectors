...
cdef unsigned long long fast_sentence_sg_neg(
    ...
    REAL_t *syn0, REAL_t *syn1neg, REAL_t *syn2subword, const int size, const np.uint32_t word_index,
    ...
    #syn0: context clue   #neu1 holds sum
    #syn1neg: word
    ...
    memset(work, 0, size * cython.sizeof(REAL_t))  #I think work is for storing gradient updates for syn0 (with all negs/target word)
    ...
    #Raj calculate sum of context clues instead of syn0[row1], use neu1
    ...
    for m in range(0, MAX_CONTEXT_SIZE):
        #count += ONEF
        if cc_set[m] != -1:
            our_saxpy(&size, &ONEF, &syn0[cc_set[m] * size], &ONE, neu1, &ONE)  #our alpha here is 1, so just adds each context clue
    ...
    for qqqq in range(0, MAX_CONTEXT_SIZE):
        if cc_set[qqqq] != -1:
            our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0[cc_set[qqqq] * size], &ONE)  #updates syn0 (context clues?)  #LHS cc word and cc sub
...
cdef unsigned long long FASTER_2_PAIRS_MEAN(
    ...
    cdef int cc_set_m
    cdef int sub_padding_num_c_int = sub_padding_num
    ...
    for m in range(0, MAX_CONTEXT_SIZE):
        ...
        if cc_set_m != -1:
            cc_count += ONEF
            our_saxpy(&size, &ONEF, &syn0[cc_set_m * size], &ONE, neu1, &ONE)  #our alpha here is 1, so just adds each context clue
    ...
    #Raj calculate subwords instead of syn0[row1]
    memset(sub_neu, 0, size * cython.sizeof(REAL_t))  #reset subword sum
    ...
    for e in range(0, subword_list_len):
        ...
        if sub_ind != sub_padding_num_c_int:
            #with gil:
            #    print(sub_ind)
            our_saxpy(&size, &ONEF, &syn2subword[sub_ind * size], &ONE, sub_neu, &ONE)  #our alpha here is 1, so just adds each context clue
            sub_count1 += ONEF
    ...
    for qqqq in range(0, MAX_CONTEXT_SIZE):
        cc_set_m = cc_set[qqqq] 
        if cc_set_m != -1:
            our_saxpy(&size, &word_locks[word2_index], work, &ONE, &syn0[cc_set_m * size], &ONE)  #updates syn0 (context clues?)  #LHS cc word and cc sub
...
#Jan 23
#3 pair but faster by subwords on left side
# (cc1 word2) (sub1 word2) (cc1 sub2)
#----------------------------------------------------------------------------------------------------------------------------------
cdef unsigned long long FASTEST_3_PAIRS_MEAN(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
    REAL_t *syn0, REAL_t *syn1neg, REAL_t *syn2subword, const int size, const np.uint32_t word_index,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work, REAL_t *sub_work, REAL_t *neu1, REAL_t *sub_neu, REAL_t *sub_neu2,
    unsigned long long next_random, REAL_t *word_locks,
    const int _compute_loss, REAL_t *_running_training_loss_param, np.uint32_t* cc_set, int* index_subword_breakdown, int subword_list_len, int sub_padding_num, int *skipped_pairs) nogil:
    ...
    cdef long long row1 = word2_index * size, row2
    cdef unsigned long long modulo = 281474976710655ULL
    cdef REAL_t f, g, label, f_dot, log_e_f_dot, cc_s_dot, f_cc_s, g_cc_s,g_cc_s_scaled, w_s_dot, f_w_s, g_w_s, cc_count, sub_count1, sub_count2, cc_inv_count = 1.0, sub_inv_count1 = 1.0, sub_inv_count2 = 1.0
    cdef np.uint32_t target_index
    cdef int d, m, qqqq, q, yyyy, sub_ind, e, e2
    cdef int cc_set_m
    cdef int sub_padding_num_c_int = sub_padding_num
    ...
    for d in range(negative+1):
        ...
        memset(sub_neu2, 0, size * cython.sizeof(REAL_t))  #reset subword sum
        ...
        for e in range(0, subword_list_len):
            ...
            sub_ind = index_subword_breakdown[(target_index * subword_list_len)+e]
 
            if sub_ind != sub_padding_num_c_int:
                    our_saxpy(&size, &ONEF, &syn2subword[sub_ind * size], &ONE, sub_neu2, &ONE)  #our alpha here is 1, so just adds each context clue
                    sub_count2 += ONEF
        ...
        sscal(&size, &sub_inv_count2, sub_neu2, &ONE)
        ...
        #cc and sub -------------------------------------------------------------
        cc_s_dot = our_dot(&size, neu1, &ONE, sub_neu2, &ONE)   
        if cc_s_dot <= -MAX_EXP or cc_s_dot >= MAX_EXP:
        ...
        our_saxpy(&size, &g_cc_s, sub_neu2, &ONE, work, &ONE)  #LHS cc sub
        ...
        g_cc_s_scaled = g_cc_s * sub_inv_count2

        for e2 in range(0, subword_list_len):
            sub_ind = index_subword_breakdown[(target_index * subword_list_len)+e2]
 
            if sub_ind != sub_padding_num_c_int:
                our_saxpy(&size, &g_cc_s_scaled, neu1, &ONE, &syn2subword[sub_ind * size], &ONE)  #our alpha here is 1, so just adds each context clue
...
cdef unsigned long long fast_sentence_sg_neg_INTERNAL_SUM(
    ...
    for qqqq in range(0, MAX_CONTEXT_SIZE):
        if cc_set[qqqq] != -1:
            our_saxpy(&size, &word_locks[word2_index], cc_work, &ONE, &syn0[cc_set[qqqq] * size], &ONE)  #updates syn0 (context clues?)  #LHS cc word and cc sub
...
def train_batch_sg_old(model, sentences, alpha, _work, _word_work, _neu1, _sub_neu, _sub_neu2, compute_loss, context_size, cc_sampling, pairs):
   ...
   # print(type(model.sub.index_subword_breakdown[0][0]))

    
    #print('start')
    #for e in range(subword_list_len):
        #print(model.sub.index_subword_breakdown[3][e])
        #print(index_subword_breakdown[(3 * subword_list_len * 2)+(2*e)])
        #print(index_subword_breakdown[(3 * subword_list_len)+e])
        #print('---')
    #print('done')
    
    #print(len(model.sub.index_subword_breakdown[0]))

    cdef int sub_padding_num = <int> model.sub.padding_number
    ...
    sub_neu2 = <REAL_t *>np.PyArray_DATA(_sub_neu2)  #Raj: added in to store subword sum    
            ...
            for i in range(idx_start, idx_end):
                ...
                for j in range(j, k):
                    ...
                    if negative:
                        ...
                        if pairs_int == 2:
                            #next_random = fast_sentence_sg_neg(negative, cum_table, cum_table_len, syn0, syn1neg, syn2subword, size, indexes[j], indexes[i], _alpha, work, word_work, neu1, sub_neu, next_random, word_locks, _compute_loss, &_running_training_loss, cc_set, index_subword_breakdown, subword_list_len, sub_padding_num, _skipped_pairs)
                            next_random = fast_sentence_sg_neg_ALL_2_PAIRS_MEAN(negative, cum_table, cum_table_len, syn0, syn1neg, syn2subword, size, indexes[j], indexes[i], _alpha, work, word_work, neu1, sub_neu, next_random, word_locks, _compute_loss, &_running_training_loss, cc_set, index_subword_breakdown, subword_list_len, sub_padding_num, _skipped_pairs)
                            
                        elif pairs_int == 3:
                            next_random = fast_sentence_sg_neg_ALL_3_PAIRS(negative, cum_table, cum_table_len, syn0, syn1neg, syn2subword, size, indexes[j], indexes[i], _alpha, work, word_work, neu1, sub_neu, next_random, word_locks, _compute_loss, &_running_training_loss, cc_set, index_subword_breakdown, subword_list_len, sub_padding_num, _skipped_pairs)
                            ...
                            next_random = FASTER_2_PAIRS_MEAN(negative, cum_table, cum_table_len, syn0, syn1neg, syn2subword, size, indexes[j], indexes[i], _alpha, work, word_work, neu1, sub_neu, next_random, word_locks, _compute_loss, &_running_training_loss, cc_set, index_subword_breakdown, subword_list_len, sub_padding_num, _skipped_pairs)

                            
                        elif pairs_int == 12:
                            next_random = FASTEST_3_PAIRS_MEAN(negative, cum_table, cum_table_len, syn0, syn1neg, syn2subword, size, indexes[j], indexes[i], _alpha, work, word_work, neu1, sub_neu, sub_neu2, next_random, word_locks, _compute_loss, &_running_training_loss, cc_set, index_subword_breakdown, subword_list_len, sub_padding_num, _skipped_pairs)
